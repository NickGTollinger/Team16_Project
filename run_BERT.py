...
if __name__ == "__main__":
    ...
    if "clickbait" in df.columns:
        ...
        # Save correctly identified clickbait headlines only (no label, just headline)
        correct_clickbait = df[(df["clickbait"] == 1) & (df["prediction"] == 1)]
        correct_clickbait[["headline"]].to_csv("correct_clickbait.csv", index=False)
        print(f"Saved correctly identified clickbait headlines to correct_clickbait.csv")
    else:
